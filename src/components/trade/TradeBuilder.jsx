    ...
    <div className="flex items-center gap-1 py-1 min-w-0">
      {tappable ? (
        <button
          ...
          className="focus-ring flex-1 flex items-center gap-1 min-w-0 text-left press"
        >
          {inner}
        </button>
      ) : (
      ...
      <button
        onClick={onRemove}
        className="focus-ring shrink-0 w-4 text-center text-text-tertiary dark:text-text-tertiary text-base leading-none font-medium"
        aria-label="Remove"
      >
        ×
      </button>
...
function TradeColumn({ label, assets, total, onRemove, onAdd, addLabel, onTapPlayer }) {
  return (
    <div className="flex-1 min-w-0">
      ...
      <button
        onClick={onAdd}
        className="focus-ring w-full mt-1.5 flex items-center justify-center gap-1 py-2 border border-dashed border-border-default dark:border-border-default text-text-secondary dark:text-text-secondary font-body text-[11px] font-semibold uppercase tracking-wide press"
      >
        {addLabel}
      </button>
    </div>
...
function PlayerRow({ player, isSelected, onTap, onInfo, onWhatsFair }) {
  ...
  return (
    ...
    >
      <button
        onClick={onTap}
        className="focus-ring flex-1 flex items-center gap-1.5 py-2.5 press text-left min-w-0"
      >
        {/* Selection indicator */}
      ...
      {onWhatsFair && (
        <button
          ...
          className="focus-ring shrink-0 px-2 py-2.5 text-text-tertiary dark:text-text-tertiary active:text-warning transition-colors"
      ...
      <button
        ...
        className="focus-ring shrink-0 px-2 py-2.5 text-text-tertiary dark:text-text-tertiary active:text-text-secondary transition-colors"
      ...
      {hasAssets && (
        <div className="flex justify-center pt-2 pb-1">
          <button
            ...
            className="focus-ring font-body text-[11px] text-text-tertiary dark:text-text-tertiary active:text-danger transition-colors"
          >
            × Clear trade
          </button>
        </div>
