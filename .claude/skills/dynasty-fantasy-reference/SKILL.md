...
| **FAAB** | Free Agent Acquisition Budget — a blind-bid dollar budget for claiming free agents (highest bid wins). | Always displayed `$XXX` (e.g. `$142`). Winning bids surface from `settings.waiver_bid` on transactions. **THIS league: `$1000` for 2026 (was `$100` 2023–25), and it RESETS TWICE A LEAGUE YEAR** — offseason, then again at the regular-season start, unspent offseason money lost. So one Sleeper season carries **two** budgets: `roster.settings.waiver_budget_used` tracks only the current period, while the transaction log spans both and routinely exceeds one budget (measured 2023–26: six manager-seasons over one, none ever over two). Any cross-season aggregate must be in **budgets**, never dollars — see CLAUDE.md League Context and Feature 11. |
