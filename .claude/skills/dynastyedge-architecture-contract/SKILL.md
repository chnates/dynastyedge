...
(`1313933520715907072`, 10-team Superflex Half-PPR). Static site on GitHub
...
permanent spinner — a real failure mode on mobile Safari. (One sanctioned
non-fetchJSON network surface: static `<img>` avatar tags in
`TeamAvatar.jsx` — images, not API calls.)
...
- `league` → `{ allRosters, myRoster, userMap, leagueInfo }` — `allRosters`
  ...
  `findPickValue` from `src/utils/pickCapital.js`), `totalValue`, FAAB fields,
