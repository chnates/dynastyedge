...
| **Missing `node_modules` masquerading as failing tests** | Every remote/cloud session starts from a fresh clone with no `node_modules`, and `npm test` does NOT say so. It prints `# tests 136 / # pass 129 / # fail 7` — which reads as a code regression. Cause: the 7 files that transitively import `react` (`tradeAnalysis.test.mjs` via `tradeAnalysis.js → recommendations.js → useLeague.js`; `matchupWeeks.test.mjs`, `transactions.test.mjs`, `sleeperDraft.test.mjs` and `draftLive.test.mjs` via their hooks) fail to *load*, so their tests never run and the total drops from **242** to 136. `npm run build` in the same state fails `sh: 1: vite: not found`. **Rule: if `npm test` doesn't say 242, run `npm ci` before debugging.** (Re-measured 2026-09-06 by moving `node_modules` aside; was 163/115 on 2026-09-04 and 120/47 on 2026-08-08. The broken-state signature has held at 136/129/7 across the last two re-measurements — it is the passing count that moves.) |
