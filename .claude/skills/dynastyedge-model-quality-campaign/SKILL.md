...
| `src/utils/tradeAnalysis.js` | `analyzeTrade` (3 layers), `getTradeVerdict`, `getCounterSuggestion`, `suggestFairPackage` | "even" at `valuePct ≤ 5` (pct of the LARGER side, rounded); hard Decline when losing `> 15%`; Accept-despite-overpay allowed up to 15% when fit is positive; Counter when winning `> 5%` but window score negative; package **assembly** window `PACKAGE_BAND` = `0.9×`–`1.15×` target, but the returned SUGGESTION is filtered through `buildFairBand` (±5%) and the wider window only feeds `alternative` (OPEN-10, 2026-09-21); counter-bridger seeks an asset worth `0.8–1.5×` the gap |
