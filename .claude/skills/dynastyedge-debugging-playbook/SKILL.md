...
| Rookie "Rk ADP" shows `—` | Rookie with no FantasyCalc rank sorts to bottom by design. The join is by `sleeperId` only (no name fallback since 2026-09-25, ROOKIE-1), so a rookie FantasyCalc hasn't mapped to his Sleeper id stays `—` rather than borrowing a namesake's value | `src/utils/rookieAdp.js` |
| Draft › Research shows "Rookie intel hasn't published yet" | Class B feed. The `rookie-intel` branch first published 2026-08-14, so this now means the fetch failed or the daily cron stopped — not the pre-first-run state. Board falls back to dynasty-value order | `src/hooks/useRookieIntel.js` (`intelFailed` latch); `check-feeds.mjs` |
